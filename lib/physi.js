    ...
    Physijs.Scene.prototype._updateScene = function( data ) {
        ...
        for ( i = 0; i < num_objects; i++ ) {
            ...
            if ( object.__dirtyPosition === false ) {
                if (object._physijs.collider && object._physijs.collider.center) {
                    var center = object._physijs.collider.center;
                    // sub the offset of the collider
                    object.position.set(
                        data[ offset + 1 ] - center.x,
                        data[ offset + 2 ] - center.y,
                        data[ offset + 3 ] - center.z
                    );
                }
                else {
                    object.position.set(
    ...
    Physijs.Scene.prototype.add = function( object ) {
        ...
        if ( object._physijs ) {
            ...
            if ( object instanceof Physijs.Vehicle ) {
                ...
                if (object._physijs.collider) {
                    var center = object._physijs.collider.center;
                    if (center) {
                        object._physijs.position.x += center.x;
                        object._physijs.position.y += center.y;
                        object._physijs.position.z += center.z;
                    }

                    var size = object._physijs.collider.size;
                    if (size) {
                        switch (object._physijs.type) {
                            case "box" : {
                                object._physijs.width = size.x;
                                object._physijs.height = size.y;
                                object._physijs.depth = size.z;
                                break;
                            }
                        }
                    }
                ...
                // console.log(object.name, ", object._physijs.collider: ", object._physijs.collider);
