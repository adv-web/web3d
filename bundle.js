...
(function() {
  ...
  NetWorkManager = (function() {
    ...
    NetWorkManager.addPrefab = function(prefab) {
      return NetWorkManager.prefabs[prefab.key] = prefab;
    };

    NetWorkManager.add = function(key, messages) {
      var data, prefab;
      prefab = NetWorkManager.prefabs[key];
      if (prefab === void 0) {
    ...
    NetWorkManager._client_connect_to_server = function() {
      ...
      return NetWorkManager.socket.on(NetWorkManager.objectUpdateEvent, function(data) {
        switch (data.action) {
          case 's':
            return NetWorkManager._spawn(data.prefabId, data.objectId, data.mess);
          case 'd':
            return NetWorkManager._destroy(data.objectId);
          case 'u':
            return NetWorkManager._update(data.objectId, data);
        }
      });
    };

    NetWorkManager._spawn = function(key, id, mess) {
      var object;
      object = NetWorkManager.prefabs[key](mess);
      if (object) {
        object.id = id;
...
// Generated by CoffeeScript 1.12.5
(function() {
  var Bullet, FirstPersonShooter, Game, GameObject, Input, NetWorkComponent, NetWorkManager,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty;
  ...
  FirstPersonShooter = (function(superClass) {
    ...
    FirstPersonShooter.prototype.afterAdded = function() {
      return Input.registerClickResponse(this._onFire);
    };
    ...
    FirstPersonShooter.prototype._onFire = function() {
      var currentTime, mess, pos, rot_x, rot_y, vel, worldPosition;
      if (!this.isLocal) {
        return;
      }
      ...
      vel = [-Math.sin(rot_y) * Math.cos(rot_x) * this.bullet_speed, Math.sin(rot_x) * this.bullet_speed, -Math.cos(rot_y) * Math.cos(rot_x) * this.bullet_speed];
      mess = {
        pos: pos,
        vel: vel
      };
      return NetWorkManager.add("bullet", mess);
    };

    FirstPersonShooter.prototype._makeBullet = function(mess) {
      var geometry, material, mesh, obj, pos, vel;
      pos = mess.pos;
      vel = mess.vel;
...
var FirstPersonController = __webpack_require__(9);
var HUD = __webpack_require__(12);
var FirstPersonShooter = __webpack_require__(10);
var TreeCollider = __webpack_require__(14);
...
function scene1(scene) {
    ...
    function player() {
        ...
        var NetWorkTransformComponent = __webpack_require__(13);
