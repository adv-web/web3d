...
(function() {
  ...
  Game = (function() {
    ...
    Game._onWindowResize = function() {
      var ref, ref1, scene;
      scene = Game.scene;
      if (scene != null) {
        if ((ref = scene._cameras[0]) != null) {
          ref.aspect = window.innerWidth / window.innerHeight;
        }
      }
      if (scene != null) {
        if ((ref1 = scene._cameras[0]) != null) {
          ref1.updateProjectionMatrix();
        }
      }
      return Game.renderer.setSize(window.innerWidth, window.innerHeight);
    };
...
(function() {
  ...
  Data = (function() {
    ...
    Data.load = function(onStart, onUpdate, onReady) {
      ...
      this._loadVox();
      return this._loadPrefab();
    };

    Data._loadVox = function() {
      return $.getJSON("vox/config.json", (function(_this) {
        return function(array) {
          ...
          return array.forEach(function(item) {
            var filename;
            filename = "vox/" + (item.filename ? item.filename : item.name) + ".vox";
            return voxParser.parse(filename).then(function(data) {
              var threeMesh;
              threeMesh = (new vox.MeshBuilder(data, {
                voxelSize: item.scale
              })).createMesh();
              Data.vox[item.name] = new Physijs.BoxMesh(threeMesh.geometry, threeMesh.material, 0);
              return _this._checkVox({
                type: "vox",
                major: 0,
                minorCount: array.length
              });
            });
          });
        };
    ...
    Data._loadPrefab = function() {
      return $.getJSON("prefab/config.json", (function(_this) {
        return function(array) {
          Data.prefab || (Data.prefab = {});
          return array.forEach(function(item) {
            var filename;
            filename = "prefab/" + (item.filename ? item.filename : item.name) + ".prefab";
            return $.getJSON(filename, function(prefab) {
              Data.prefab[item.name] = prefab;
              return _this._checkPrefab({
                type: "prefab",
                major: 1,
                minorCount: array.length
              });
    ...
    Data._voxLoadCount = 0;

    Data._checkVox = function(data) {
      data.minor = Data._voxLoadCount;
      if (typeof Data._onUpdate === "function") {
        Data._onUpdate(data);
      }
      Data._voxLoadCount += 1;
      if (Data._voxLoadCount === data.minorCount) {
        return this._checkGame();
      }
    };

    Data._prefabLoadCount = 0;

    Data._checkPrefab = function(data) {
      data.minor = Data._prefabLoadCount;
      if (typeof Data._onUpdate === "function") {
        Data._onUpdate(data);
      }
      Data._prefabLoadCount += 1;
      if (Data._prefabLoadCount === data.minorCount) {
        return this._checkGame();
      }
    };

    Data._majorLoadCount = 0;

    Data._checkGame = function() {
      Data._majorLoadCount += 1;
      if (Data._majorLoadCount === Data.MAJOR) {
        return typeof Data._onReady === "function" ? Data._onReady() : void 0;
      }
    };
...
(function() {
  ...
  NetWorkManager = (function() {
    ...
    NetWorkManager.init = function(scene, playerPrefab, serverAddress) {
      ...
      NetWorkManager.userInfoUpdate = "user.info.update";
    ...
    NetWorkManager.setUserInfo = function(userInfo1, updateCallback) {
      NetWorkManager.userInfo = userInfo1;
      NetWorkManager.updateCallback = updateCallback;
    };

    NetWorkManager.updateUserInfo = function(userInfo) {
      if (userInfo == null) {
        userInfo = NetWorkManager.userInfo;
      }
      return NetWorkManager.socket.emit(NetWorkManager.userInfoUpdate, userInfo);
    };

    NetWorkManager.setSpawnPoint = function(spawnPoint) {
      NetWorkManager.spawnPoint = spawnPoint;
    };

    NetWorkManager.setGameTimeListener = function(_gameTimeListener) {
      NetWorkManager._gameTimeListener = _gameTimeListener;
    };

    NetWorkManager.setGameEndListener = function(_gameEndListener) {
      NetWorkManager._gameEndListener = _gameEndListener;
    };
    ...
    NetWorkManager.update = function(object, message, callback) {
      ...
      data = {
        action: 'u',
        objectId: object.isGameObject ? object.id : object,
        message: JSON.stringify(message),
        createTime: time
      };
      if (callback != null) {
    ...
    NetWorkManager.remove = function(object, callbackFunction) {
      ...
      delete NetWorkManager.gameObjects[object.id];
      event = "_destroy_callback_" + object.id;
      NetWorkManager.socket.on(event, callbackFunction);
      data = {
        action: 'd',
        callback: event,
        objectId: object.id
    ...
    NetWorkManager._client_connect_to_server = function() {
      ...
      NetWorkManager.socket.on('message', NetWorkManager._client_onnetmessage);
      NetWorkManager.socket.on(NetWorkManager.userInfoUpdate, NetWorkManager._user_info_update);
      return NetWorkManager.socket.on(NetWorkManager.objectUpdateEvent, function(data) {
        switch (data.action) {
          case 's':
            return NetWorkManager._spawn(data);
          case 'd':
            return NetWorkManager._destroy(data);
          case 'u':
            return NetWorkManager._update(data);
        }
      });
    };

    NetWorkManager._user_info_update = function(data) {
      return NetWorkManager.updateCallback(data);
    };
    ...
    NetWorkManager._destroy = function(data) {
      var id;
      id = data.objectId;
      if (NetWorkManager.gameObjects[id] != null) {
    ...
    NetWorkManager._update = function(data) {
      ...
      if (data.objectId === "global-message") {
        ...
        if (NetWorkManager.players.self.id === data.objectId) {
          NetWorkManager.players.self.broadcast(message);
        }
        if ((ref1 = NetWorkManager.players.others[data.objectId]) != null) {
          ref1.broadcast(message);
        }
        if (NetWorkManager._callbacks[data.createTime] != null) {
    ...
    NetWorkManager._onGlobalMessage = function(message) {
    ...
    NetWorkManager._client_onconnected = function(data) {
      ...
      NetWorkManager.players.self.online = true;
      return NetWorkManager.updateUserInfo();
    };

    NetWorkManager._client_ondisconnect = function(data) {
      return NetWorkManager.scene.remove(NetWorkManager.players.self);
    };

    NetWorkManager._client_onserverupdate_recieved = function(data) {};

    NetWorkManager._client_endGame = function() {
      console.log("game end");
      return typeof NetWorkManager._gameEndListener === "function" ? NetWorkManager._gameEndListener() : void 0;
    };

    NetWorkManager._client_updateTime = function(time) {
      console.log("Time: ", time);
      return typeof NetWorkManager._gameTimeListener === "function" ? NetWorkManager._gameTimeListener(time) : void 0;
    };
      ...
      switch (command) {
        case 's':
          switch (subcommand) {
            ...
            case 'ol':
              return NetWorkManager._client_onotherleftgame(commanddata);
            case 'r':
              return NetWorkManager._client_onreadygame(commanddata);
            case 's':
              return NetWorkManager._client_onstartgame(commanddata);
            case 'e':
              return NetWorkManager._client_endGame(commanddata);
            case 'ut':
              return NetWorkManager._client_updateTime(commanddata);
            case 'p':
    ...
    NetWorkManager._client_onotherleftgame = function(id) {
      if (NetWorkManager.players.others[id]) {
        NetWorkManager.scene.remove(NetWorkManager.players.others[id]);
      }
      delete NetWorkManager.players.others[id];
    ...
    NetWorkManager._client_onhostgame = function(game_id) {
      ...
      NetWorkManager.players.self.host = true;
    ...
    NetWorkManager._client_initLocalPlayer = function() {
      ...
      player = NetWorkManager._get_player();
...
(function() {
  ...
  Input = (function() {
    ...
    Input._onclick = function() {
      var i, len, ref, res, results;
      ref = Input._clickResponses;
      results = [];
    ...
    Input.registerClickResponse = function(res) {
      return Input._clickResponses.push(res);
    };

    Input.removeClickResponse = function(res) {
      return Input._clickResponses.remove(res);
    };
    ...
    document.body.onclick = Input._onclick;
...
// Generated by CoffeeScript 1.12.6
(function() {
  var AudioListener, AudioSource, Component,
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty;

  Component = __webpack_require__(0);

  AudioListener = __webpack_require__(9);

  AudioSource = (function(superClass) {
    extend(AudioSource, superClass);
    ...
    function AudioSource(options) {
      ...
      if (this.audioclip) {
        new THREE.AudioLoader().load(this.audioclip, (function(_this) {
          return function(buffer) {
            sound.setBuffer(buffer);
            sound.setLoop(_this.loop);
...
(function() {
  ...
  GameObject = (function() {
    ...
    GameObject.prototype.beforeRemoved = function() {
      var comp, key, ref, results;
      ref = this.components;
      results = [];
      for (key in ref) {
        comp = ref[key];
        results.push(typeof comp.beforeRemoved === "function" ? comp.beforeRemoved() : void 0);
      }
      return results;
    };
...
// Generated by CoffeeScript 1.12.6
(function() {
  var AudioListener, Component, Game,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty;
  ...
  AudioListener = (function(superClass) {
    extend(AudioListener, superClass);

    module.exports = AudioListener;

    AudioListener._listener = new THREE.AudioListener();

    function AudioListener() {
      this.update = bind(this.update, this);
      this.onStartLocalPlayer = bind(this.onStartLocalPlayer, this);
      AudioListener.__super__.constructor.call(this, "AudioListener");
    }

    AudioListener.prototype.onStartLocalPlayer = function() {
      var ref;
      this.camera = Game.scene._cameras[0];
      return (ref = this.camera) != null ? ref.add(AudioListener._listener) : void 0;
    };

    AudioListener.prototype.update = function() {
      if (this.camera !== Game.scene._cameras[0]) {
        this.camera = Game.scene._cameras[0];
        return this.camera.add(AudioListener._listener);
      }
    };
...
(function() {
  ...
  ExplodeAnimation = (function(superClass) {
    ...
    ExplodeAnimation.prototype.update = function() {
      ...
      if (Date.now() - this._startTime > this.duration) {
...
// Generated by CoffeeScript 1.12.6
(function() {
  var AudioListener, AudioSource, BoxCollider, Bullet, Camera, Data, FirstPersonObserver, FirstPersonShooter, GUIDat, GameObject, HUD, NetWorkComponent, NetWorkTransform, Pumpkin, PumpkinTree, Scene, Tank, TreeCollision, VehicleController,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };

  Data = __webpack_require__(2);

  GameObject = __webpack_require__(8);

  Bullet = __webpack_require__(13);

  Camera = __webpack_require__(14);

  FirstPersonObserver = __webpack_require__(15);

  VehicleController = __webpack_require__(24);

  FirstPersonShooter = __webpack_require__(16);

  GUIDat = __webpack_require__(17);

  HUD = __webpack_require__(18);

  NetWorkComponent = __webpack_require__(4);

  NetWorkTransform = __webpack_require__(19);

  TreeCollision = __webpack_require__(23);

  BoxCollider = __webpack_require__(25);

  AudioListener = __webpack_require__(9);

  AudioSource = __webpack_require__(7);

  Tank = __webpack_require__(22);

  Pumpkin = __webpack_require__(20);

  PumpkinTree = __webpack_require__(21);
  ...
  Scene = (function() {
    ...
    Scene.prototype.spawn = function(prefab, position, rotation) {
      var mesh, obj, prototype, ref;
      if (position == null) {
      ...
      prototype = (function() {
        switch (prefab.meshType) {
          case "vox":
            return Data.vox[prefab.mesh];
          case "code":
            return eval(prefab.mesh);
        }
      })();
      mesh = prototype.clone();
      if (prefab.mass) {
        mesh.mass = prefab.mass >= 0 ? prefab.mass : prototype.mass;
      } else {
        mesh.mass = 0;
      }
      ...
      if ((ref = prefab.components) != null) {
        ref.forEach((function(_this) {
          return function(comp) {
            var param;
            param = comp.param ? comp.param : "";
            return eval("obj.addComponent(new " + comp.name + "(" + param + "))");
          };
        })(this));
      }
      this.add(obj);
...
// Generated by CoffeeScript 1.12.6
(function() {
  var $, Component, GameTimeCountdown, NetWorkMananger,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty;
  ...
  NetWorkMananger = __webpack_require__(3);

  $ = __webpack_require__(6);

  GameTimeCountdown = (function(superClass) {
    extend(GameTimeCountdown, superClass);

    module.exports = GameTimeCountdown;

    GameTimeCountdown.prototype.total = 180;

    function GameTimeCountdown(total) {
      this.total = total != null ? total : 180;
      this._onGameEnd = bind(this._onGameEnd, this);
      this._onGameTimeChange = bind(this._onGameTimeChange, this);
      this.beforeRemoved = bind(this.beforeRemoved, this);
      this.afterAdded = bind(this.afterAdded, this);
    ...
    GameTimeCountdown.prototype.afterAdded = function() {
      NetWorkMananger.setGameTimeListener(this._onGameTimeChange);
      return NetWorkMananger.setGameEndListener(this._onGameEnd);
    };

    GameTimeCountdown.prototype.beforeRemoved = function() {
      NetWorkMananger.setGameTimeListener(null);
      return NetWorkMananger.setGameEndListener(null);
    };

    GameTimeCountdown.prototype._onGameTimeChange = function(time) {
      time = parseInt(time);
      $(".gametime-data").html(time + " <span>s</span>");
      if (time <= 120) {
        this.grounds[19].mesh.mass = 0.5;
        this.grounds[18].mesh.mass = 0.5;
        this.grounds[9].mesh.mass = 0.5;
      }
      if (time <= 80) {
        this.grounds[22].mesh.mass = 0.5;
        this.grounds[21].mesh.mass = 0.5;
        this.grounds[12].mesh.mass = 0.5;
      }
      if (time <= 50) {
        this.grounds[17].mesh.mass = 0.5;
        this.grounds[16].mesh.mass = 0.5;
        this.grounds[7].mesh.mass = 0.5;
      }
      if (time <= 20) {
        this.grounds[24].mesh.mass = 0.5;
        this.grounds[23].mesh.mass = 0.5;
        return this.grounds[14].mesh.mass = 0.5;
      }
    };
...
//# sourceMappingURL=GameTimeCountdown.js.map
...
// Generated by CoffeeScript 1.12.6
(function() {
  var AudioSource, Bullet, Component, ExplodeAnimation, Game,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty,
    slice = [].slice;
  ...
  ExplodeAnimation = __webpack_require__(10);
  ...
  Bullet = (function(superClass) {
    ...
    Bullet.prototype.onCollision = function(other_mesh, linear_velocity, angular_velocity) {
      if (!this._explode) {
        this._explode = true;
    ...
    Bullet.prototype._launch = function(mass, velocity) {
      var ref, ref1;
      AudioSource.play('/client/audio/fire.wav', 1);
...
// Generated by CoffeeScript 1.12.6
(function() {
  var Component, FirstPersonObserver,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty;
  ...
  FirstPersonObserver = (function(superClass) {
    var PI_2;

    extend(FirstPersonObserver, superClass);

    module.exports = FirstPersonObserver;

    PI_2 = Math.PI / 2;

    FirstPersonObserver.prototype.sensitivity = 1;

    function FirstPersonObserver(options) {
      if (options == null) {
        options = {};
      }
      ...
      this.beforeRemoved = bind(this.beforeRemoved, this);
      this.afterAdded = bind(this.afterAdded, this);
      FirstPersonObserver.__super__.constructor.call(this, "FirstPersonObserver");
      this.sensitivity = options.sensitivity != null ? options.sensitivity : 1;
    }

    FirstPersonObserver.prototype.afterAdded = function() {
      ...
      return document.addEventListener('mousemove', this._onMouseMove, false);
    };

    FirstPersonObserver.prototype.beforeRemoved = function() {
      return document.removeEventListener('mousemove', this._onMouseMove, false);
    };

    FirstPersonObserver.prototype._onMouseMove = function(event) {
    ...
    return FirstPersonObserver;

  })(Component);

}).call(this);

//# sourceMappingURL=FirstPersonObserver.js.map
...
// Generated by CoffeeScript 1.12.6
(function() {
  var AudioSource, Data, FirstPersonShooter, Input, NetWorkComponent, NetWorkManager,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty;
  ...
  NetWorkManager = __webpack_require__(3);
  ...
  FirstPersonShooter = (function(superClass) {
    ...
    FirstPersonShooter.prototype.enabled = true;

    FirstPersonShooter.prototype.cooldown = 5000;

    FirstPersonShooter.prototype.bullet_speed = 30;

    function FirstPersonShooter(options) {
      if (options == null) {
        options = {};
      }
      ...
      this.cooldown = options.cooldown != null ? options.cooldown : 5000;
      this.bullet_speed = options.bullet_speed != null ? options.bullet_speed : 30;
    ...
    FirstPersonShooter.prototype.afterAdded = function() {
      this._tank = this.gameObject.getComponent("Tank");
      Input.registerClickResponse(this._onFire);
      return this.enabled = true;
    };

    FirstPersonShooter.prototype.beforeRemoved = function() {
      return Input.removeClickResponse(this._onFire);
    };

    FirstPersonShooter.prototype.update = function() {
      var timeleft;
      timeleft = this.cooldown - (Date.now() - this._lastFireTime);
      if (timeleft < -100) {
        return;
      }
      if (timeleft < 0) {
        ...
        $("#aim").hide();
        return $("#coldtime").show();
      }
    };

    FirstPersonShooter.prototype._onFire = function() {
      var currentTime, direction, pos, px, py, pz, vx, vy, vz, worldPosition;
      if (!this.enabled) {
        return;
      }
      if (!this.isLocal) {
      ...
      direction = this.gameObject.mesh.getWorldDirection();
      px = worldPosition.x - direction.x * 0.9;
      py = worldPosition.y + 0.3 - direction.y * 0.9;
      pz = worldPosition.z - direction.z * 0.9;
      pos = new THREE.Vector3(px, py, pz);
      vx = -direction.x * this.bullet_speed;
      vy = -direction.y * this.bullet_speed;
      vz = -direction.z * this.bullet_speed;
      NetWorkManager.spawn(Data.prefab.bullet, {
        position: pos
      }, (function(_this) {
        return function(obj) {
      ...
      $("#coldtime-sec").text(Math.ceil(this.cooldown / 1000));
      $("#aim").hide();
      return $("#coldtime").show();
    };
...
(function() {
  ...
  GUIDat = (function(superClass) {
    extend(GUIDat, superClass);

    module.exports = GUIDat;

    function GUIDat() {
      this.onStartLocalPlayer = bind(this.onStartLocalPlayer, this);
      GUIDat.__super__.constructor.call(this, "GUIDat");
    }

    GUIDat.prototype.onStartLocalPlayer = function() {
      var bc, fpc, nwtm, vc;
      this.gui = new dat.GUI();
      fpc = this.gameObject.getComponent('FirstPersonObserver');
      this.gui.add(fpc, 'sensitivity').min(0).step(0.5);
      vc = this.gameObject.getComponent('VehicleController');
      this.gui.add(vc, 'move_velocity').min(0).step(0.5);
      this.gui.add(vc, 'rotate_velocity').min(0).step(0.5);
      nwtm = this.gameObject.getComponent("NetWorkTransform");
      this.gui.add(nwtm, 'networkSendRate').min(1).max(50).step(1);
      bc = this.gameObject.getComponent("BoxCollider");
      return this.gui.add(bc, 'activateMoveTime').min(1).max(100).step(1);
    };
...
//# sourceMappingURL=GUIDat.js.map
...
(function() {
  ...
  HUD = (function(superClass) {
    ...
    HUD.prototype.afterAdded = function() {
      ...
      this._hud.style.opacity = 0.5;
      this._hud.style.top = '0px';
...
// Generated by CoffeeScript 1.12.6
(function() {
  var NetWorkComponent, NetWorkManager, NetWorkTransform,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty;
  ...
  NetWorkTransform = (function(superClass) {
    extend(NetWorkTransform, superClass);
    ...
    function NetWorkTransform() {
      ...
      this.update = bind(this.update, this);
      this.onStartLocalPlayer = bind(this.onStartLocalPlayer, this);
      NetWorkTransform.__super__.constructor.call(this, "NetWorkTransform");
      this.networkSendRate = 1;
      this._networkSend = 0;
    }

    NetWorkTransform.prototype.onStartLocalPlayer = function() {
      var socket;
      if (this.networkSendRate <= 0) {
        return;
      }
      this.event = "nwtc." + this.gameObject.mesh.name;
      socket = NetWorkManager.socket;
      return socket.on(this.event, (function(_this) {
        return function(data) {
          var otherObject, player;
          ...
          if (player) {
            ...
            if (otherObject) {
              ...
              return otherObject.mesh.__dirtyPosition = true;
            }
          }
        };
      })(this));
    ...
    NetWorkTransform.prototype.update = function() {
      if (!this.isLocal) {
        return;
      }
      this._networkSend++;
      if (this._networkSend >= this.networkSendRate) {
        this.networkSend();
        return this._networkSend = 0;
      }
    };

    NetWorkTransform.prototype.networkSend = function() {
...
//# sourceMappingURL=NetWorkTransform.js.map
...
// Generated by CoffeeScript 1.12.6
(function() {
  var Component, ExplodeAnimation, Game, Pumpkin,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty;
  ...
  Pumpkin = (function(superClass) {
    extend(Pumpkin, superClass);
    ...
    function Pumpkin() {
      this._onExplodeFinish = bind(this._onExplodeFinish, this);
      this.onCollision = bind(this.onCollision, this);
      Pumpkin.__super__.constructor.call(this, "Pumpkin");
      this._explode = false;
    }

    Pumpkin.prototype.onCollision = function(other_mesh, linear_velocity, angular_velocity) {
      if (other_mesh.name !== "player") {
        return;
      }
      if (!this._explode) {
        this._explode = true;
        this.gameObject.addComponent(new ExplodeAnimation(this._onExplodeFinish));
        this.gameObject.mesh = null;
        return other_mesh.gameObject.getComponent("Tank").getExp(500);
      }
    };

    Pumpkin.prototype._onExplodeFinish = function() {
      return Game.scene.remove(this.gameObject);
    };
...
// Generated by CoffeeScript 1.12.6
(function() {
  var $, Component, Data, Game, PumpkinTree,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty;
  ...
  PumpkinTree = (function(superClass) {
    extend(PumpkinTree, superClass);

    module.exports = PumpkinTree;

    function PumpkinTree() {
      this.update = bind(this.update, this);
      PumpkinTree.__super__.constructor.call(this, "PumpkinTree");
      this._nextSpawnTime = 150;
    }

    PumpkinTree.prototype.update = function() {
      var time, x, z;
      time = parseInt($(".gametime-data").html());
      if (time !== this._nextSpawnTime) {
        return;
      }
      x = this.gameObject.mesh.position.x + Math.random() - 0.5;
      z = this.gameObject.mesh.position.z + Math.random() - 0.5;
      Game.scene.spawn(Data.prefab.pumpkin, new THREE.Vector3(x, 1, z));
      return this._nextSpawnTime -= 30;
    };
...
//# sourceMappingURL=PumpkinTree.js.map
...
// Generated by CoffeeScript 1.12.6
(function() {
  var $, Data, Input, NetWorkComponent, NetWorkManager, Tank,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty,
    slice = [].slice;
  ...
  Tank = (function(superClass) {
    ...
    Tank.LEVEL = [null, "MT", "HT", "TD", "LT", "SPG"];
    ...
    function Tank() {
      ...
      this._changeMesh = bind(this._changeMesh, this);
      this.getExp = bind(this.getExp, this);
      this.update = bind(this.update, this);
      this.receive = bind(this.receive, this);
      this.onCollision = bind(this.onCollision, this);
      Tank.__super__.constructor.call(this, "Tank");
      this.userInfo = document.userInfo;
      this._respawnTime = 0;
    }
    ...
    Tank.prototype.onCollision = function(other_mesh, linear_velocity, angular_velocity) {
      ...
      NetWorkManager.update(other_mesh.gameObject.reqPlayerId, {
        method: "deal_damage",
        damage: damage,
        killed: this.userInfo.hp === 0
      });
      if (this.userInfo.hp === 0) {
        this._respawn();
      }
    ...
    Tank.prototype.receive = function() {
      var args, data, profit;
      args = 1 <= arguments.length ? slice.call(arguments, 0) : [];
      data = args[0];
      if (data.method === "deal_damage") {
    ...
    Tank.prototype.update = function() {
      ...
      if (Input.isPressed('R') && Date.now() - this._respawnTime > Tank.GOD_TIME) {
        this._respawn();
      }
    ...
    Tank.prototype.getExp = function(exp) {
      ...
      this.userInfo.exp = realEXP / Tank.HP[this.userInfo.type];
      if (this.userInfo.exp > 1) {
        this.userInfo.exp = 1;
      }
      return document.setUserInfo(this.userInfo);
    };
    ...
    Tank.prototype._updateInfoByLevel = function() {
      var type;
      type = this.userInfo.type = Tank.LEVEL[this.userInfo.level];
      this.userInfo.power = Tank.DAMAGE[type];
      return this.userInfo.equipment = Tank.NAME[type];
    };
...
// Generated by CoffeeScript 1.12.6
(function() {
  var Component, NetWorkManager, TreeCollision,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty;
  ...
  TreeCollision = (function(superClass) {
    extend(TreeCollision, superClass);

    module.exports = TreeCollision;

    function TreeCollision() {
      this.onCollision = bind(this.onCollision, this);
      TreeCollision.__super__.constructor.call(this, "TreeCollision");
    ...
    TreeCollision.prototype.onCollision = function(other_mesh, linear_velocity, angular_velocity) {
      if (other_mesh.name === "bullet") {
        PlayerInfo.hitCount += 1;
        return NetWorkManager.remove(this.gameObject, (function(_this) {
...
//# sourceMappingURL=TreeCollision.js.map
...
// Generated by CoffeeScript 1.12.6
(function() {
  var Input, NetWorkComponent, VehicleController,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty;
  ...
  VehicleController = (function(superClass) {
    extend(VehicleController, superClass);

    module.exports = VehicleController;

    VehicleController.prototype.enabled = true;

    VehicleController.prototype.move_velocity = 3;

    VehicleController.prototype.rotate_velocity = 1;
    ...
    function VehicleController(options) {
      ...
      this.update = bind(this.update, this);
      this.afterAdded = bind(this.afterAdded, this);
      VehicleController.__super__.constructor.call(this, "VehicleController");
      this.enabled = false;
      this.move_velocity = options.move_velocity != null ? options.move_velocity : 3;
      this.rotate_velocity = options.rotate_velocity != null ? options.rotate_velocity : 1;
    }

    VehicleController.prototype.afterAdded = function() {
      return this.enabled = true;
    };

    VehicleController.prototype.update = function(deltaTime) {
      var hor, ver;
      if (!this.enabled) {
        return;
      }
      ...
      this.gameObject.mesh.translateZ(ver * this.move_velocity * deltaTime / 1000);
      this.gameObject.mesh.__dirtyPosition = true;
      this.gameObject.mesh.rotateY(-hor * this.rotate_velocity * deltaTime / 1000);
...
(function() {
  ...
  Collider = __webpack_require__(26);
...
(function() {
  ...
  Collider = (function(superClass) {
    ...
    Collider.prototype.afterAdded = function() {
      this.gameObject.mesh.addEventListener('collision', this._onCollision.bind(this));
      return this.gameObject.mesh.isTrigger = this.isTrigger;
    };

    Collider.prototype.beforeRemoved = function() {
      var ref;
      return (ref = this.gameObject.mesh) != null ? ref.removeEventListener('collision', this._onCollision.bind(this)) : void 0;
    };
...
var NetWorkManager = __webpack_require__(3);
